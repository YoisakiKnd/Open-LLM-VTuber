...
export const ModeProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    if (isElectron && window.electron) {
      const handlePreModeChange = (newMode: ModeType) => {
        // Use double requestAnimationFrame to ensure UI is ready
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            // Tell main process we're ready for the actual mode change
            window.api.rendererReadyForModeChange(newMode);
          });
        });
      };

      const handleModeChanged = (newMode: ModeType) => {
        setModeState(newMode);
        // After mode is set, tell main process the UI has been updated
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            window.api.modeChangeRendered();
          });
        });
      };

      const unsubscribePreMode =
        window.api.onPreModeChanged(handlePreModeChange);
      const unsubscribeMode = window.api.onModeChanged(handleModeChanged);
