...
export class WindowManager {
  ...
  constructor() {
    ipcMain.on("renderer-ready-for-mode-change", (_event, newMode) => {
      // The renderer signals readiness after a double requestAnimationFrame;
      // no fixed delay is needed for the window property changes below.
      if (newMode === "pet") {
    ...
    // Recompute the pet-mode bounds whenever displays change, so the avatar
    // window keeps spanning all monitors without a fixed polling delay.
    screen.on("display-added", () => this.refreshPetBounds());
    screen.on("display-removed", () => this.refreshPetBounds());
    screen.on("display-metrics-changed", () => this.refreshPetBounds());
  ...
  private setupWindowEvents(): void {
    ...
    this.window.webContents.on("will-navigate", (event, url) => {
      const isAllowed =
        ...
        url.startsWith("file:") ||
        (this.gatewayUrl !== null && url.startsWith(this.gatewayUrl));
  ...
  private loadContent(): void {
    ...
    if (is.dev && process.env.ELECTRON_RENDERER_URL) {
      ...
      // Packaged builds load the frontend from the gateway's static hosting:
      // the renderer is same-origin with the WebSocket and the settings API,
      // avoiding Origin/CORS failures on `file://` pages.
